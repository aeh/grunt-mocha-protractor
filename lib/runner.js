...
module.exports = function (grunt, fileGroup, browser, options, next) {
  ...
  // sauce labs.  assume sauce connect is running on port 4445
    ...
    if (browser.base && browser.base.toLowerCase() === 'saucelabs') {
    ...
    } else if (browser.browserName && browser.browserName.toLowerCase() === 'phantomjs') {
      capabilities = protractor.Capabilities[browserName]();
      capabilities.merge({
        'phantomjs.binary.path': browser['phantomjs.binary.path'],
        'phantomjs.ghostdriver.cli.args': browser['phantomjs.ghostdriver.cli.args'],
        browserName: browser.browserName
      });
    }
